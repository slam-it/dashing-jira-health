...
def scopeChange(contents)
  totalPoints = contents['allIssuesEstimateSum']['value']
  addedStories = contents['issueKeysAddedDuringSprint'].map { |issue| issue[0] }
  startingPoints = contents['completedIssues'].select { |issue| issue['estimateStatistic']['statFieldId'].eql?('customfield_10007') }.reject { |issue| issue['estimateStatistic']['statFieldValue']['value'].nil? || addedStories.include?(issue['key']) }.map { |issue| issue['estimateStatistic']['statFieldValue']['value'] }.reduce(:+)
  startingPoints += contents['incompletedIssues'].select { |issue| issue['estimateStatistic']['statFieldId'].eql?('customfield_10007') }.reject { |issue| issue['estimateStatistic']['statFieldValue']['value'].nil? || addedStories.include?(issue['key']) }.map { |issue| issue['estimateStatistic']['statFieldValue']['value'] }.reduce(:+)

  (((totalPoints - startingPoints) / totalPoints) * 100).ceil
end
...
def points(contents, statusIds)
  contents.select { |issue| issue['estimateStatistic']['statFieldId'].eql?('customfield_10007') && statusIds.include?(issue['statusId']) }.map { |issue| issue['estimateStatistic']['statFieldValue']['value'] }.reject(&:nil?).reduce(0) { |sum, num| sum + num }
end
...
SCHEDULER.every '30m', first_in: 0 do |_job|
  ...
  # 1 => 'Open', 10003 => 'Analyzing', 10014 => 'Reviewing', 10016 => 'Ready For Sprint', 10027 => 'Ready For Poker', 10031 => 'On hold/blocked'
  toDo = points(report['contents']['incompletedIssues'], %w(1 10003 10014 10016 10027 10031))
  ...
  inProgress = points(report['contents']['incompletedIssues'], %w(3 10005 10007 10024 10028 10029 10030))
  # 6 => 'Closed'
  done = points(report['contents']['completedIssues'], ['6'])
  ...
  flagged = report['contents']['incompletedIssues'].count { |issue| issue['flagged'] }
  blocker = report['contents']['incompletedIssues'].count { |issue| issue['priorityName'].eql?('Blocker') }
